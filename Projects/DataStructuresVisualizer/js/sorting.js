...
function clearTransientVisuals() {
  ...
  document.querySelectorAll('#bars .bar.active').forEach(b => b.classList.remove('active'));
  document.querySelectorAll('#bars .bar.sorted').forEach(b => b.classList.remove('sorted'));
}
...
function startSort() {
  // Reset everything
  baseArray = [...originalArray]; // Restore original array
  depthMap = new Map();
  clearTransientVisuals();
  resetPlayback();
  ...
  // Reset to original state for visualization
...
function renderMergeBuffer(buf, range) {
  ...
  buf.forEach(v => container.append(
    el('div', { className: 'merge-cell' }, v)
  ));
