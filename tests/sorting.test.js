...
test.each([bubbleSortSteps, insertionSortSteps, mergeSortSteps, quickSortSteps])('%p handles empty, single, duplicate, and negative values', (sorter) => {
  expect(sorter([]).result).toEqual([]);
  expect(sorter([4]).result).toEqual([4]);
  expect(sorter([3, -1, 3, 0, -1]).result).toEqual([-1, -1, 0, 3, 3]);
});

test.each([bubbleSortSteps, insertionSortSteps, mergeSortSteps, quickSortSteps])('%p does not mutate its input', (sorter) => {
  const input = [3, 2, 1];
  sorter(input);
